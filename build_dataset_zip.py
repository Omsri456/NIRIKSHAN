...
README_CONTENT = """# NIRIKSHAN - ML Training Datasets & Artifacts Bundle
=====================================================
...
def create_zips():
    ...
    # 1. Dataset-only ZIP
    zip1_path = os.path.join(base_dir, "nirikshan_ml_datasets.zip")
    print(f"Creating {os.path.basename(zip1_path)}...")
    t0 = time.time()
    with zipfile.ZipFile(
        zip1_path, "w", compression=zipfile.ZIP_DEFLATED, compresslevel=6
    ) as z:
        z.writestr("README_ML_DATASETS.md", README_CONTENT)

        # ML_data
        for f in sorted(os.listdir(ml_data_dir)):
            fp = os.path.join(ml_data_dir, f)
            if os.path.isfile(fp):
                z.write(fp, os.path.join("ML_data", f))
        ...
        for f in sorted(os.listdir(processed_dir)):
            ...
            if os.path.isfile(fp):
                z.write(fp, os.path.join("data", "processed", f))

    sz1 = os.path.getsize(zip1_path) / (1024 * 1024)
    print(f"Successfully created {zip1_path} ({sz1:.2f} MB in {time.time()-t0:.1f}s)")

    # 2. Complete Bundle (Datasets + Models)
    zip2_path = os.path.join(base_dir, "nirikshan_ml_full_bundle.zip")
    print(f"Creating {os.path.basename(zip2_path)}...")
    t0 = time.time()
    with zipfile.ZipFile(
        zip2_path, "w", compression=zipfile.ZIP_DEFLATED, compresslevel=6
    ) as z:
        z.writestr("README_ML_DATASETS.md", README_CONTENT)
        ...
        for f in sorted(os.listdir(models_dir)):
            ...
            if os.path.isfile(fp):
                z.write(fp, os.path.join("ml", "models", f))

    sz2 = os.path.getsize(zip2_path) / (1024 * 1024)
    print(f"Successfully created {zip2_path} ({sz2:.2f} MB in {time.time()-t0:.1f}s)")
